...
export class QueueService {
    ...
    async initiateQueues() {
        this.defaultQueue = new Queue(QUEUE_NAMES.DEFAULT, QueueService.QUEUE_OPTIONS);
        this.queues[QUEUE_NAMES.DEFAULT] = this.defaultQueue;

        this.campaignQueue = new Queue(QUEUE_NAMES.CAMPAIGN, QueueService.QUEUE_OPTIONS);
        this.queues[QUEUE_NAMES.CAMPAIGN] = this.campaignQueue
    }

    async instantiateWorkers() {
        this.defaultQueueWorker = new Worker(QUEUE_NAMES.DEFAULT, async (job: Job) => {
            switch(job.name) {
                case JobType.SEND_EMAIL:
                    try {
                        ...
                        await emailService.sendEmail(emailData, Number(campaignId)) ;   
        ...
        this.campaignWorker = new Worker(QUEUE_NAMES.CAMPAIGN, async(job: Job) => {
            switch(job.name){
                case JobType.SEND_EMAIL:
                    let {campaignId, emailCampaignId, delay, templateId, content, ...emailData} = job.data 
                    let html = content ;
                    let type = 'text'
                    if (templateId) {
                        const templateRepository = container.get<IMessageTemplateRepository>(TYPES.IMessageTemplateRepository)
                    ...
                    if (job.data.finalEmail) {
                        ...
                        this.campaignQueue.add(JobType.UPDATE_EMAIL_CAMPAIGN_STATUS, {emailCampaignId, status: "completed"})
                    } else {
                        console.log(job.data.to)
                    }

                    const emailService = new EmailService() ;
                    await emailService.sendEmail({...emailData, html, type}, campaignId)
                    break ;
                case JobType.UPDATE_EMAIL_CAMPAIGN_STATUS:
                    try {
                        const emailCampaignId = job.data.emailCampaignId 
                        const emailCampaignRepository =container.get<IEmailCampaignRepository>(TYPES.IEmailCampaignRepository)

                        await emailCampaignRepository.update(emailCampaignId,{status: job.data.status})
                ...
                case JobType.SEND_EMAIL_TO_TARGETLIST:
                    try {
                        const {targetlistIds, campaignId, emailCampaignId, startDate, ...emailData} = job.data
                        console.log(new Date(startDate).toLocaleDateString())
                        console.log(new Date().toLocaleDateString())
                        const delay = (new Date(startDate).getTime() - new Date().getTime())

                        console.log(delay)
                        console.log(targetlistIds, campaignId, emailCampaignId, startDate, emailData)
                        const clientEmails: Set<string> = new Set([])
                        const targetlistRepository = container.get<ITargetListRepository>(TYPES.ITargetListRepository)

                        for (let id of targetlistIds) {
                            console.log(Number(id))
                            let targetlist = await targetlistRepository.getById(Number(id))
                            if (targetlist) {
                                let clients = await targetlist.getClients()
                                clients.forEach((item: Client) => {
                                    let email:string = item.getDataValue("email")
                        ...
                        if (clientEmails.size === 0) {
                            ...
                            clientEmails.forEach((email: string, index: string) => {
                                console.log("Send email to ", email, "after ", delay, " (s)")
                                console.log(new Date(startDate).toLocaleString())
                                console.log(new Date().toLocaleString())
                                count++ ;
                                this.campaignQueue.add(JobType.SEND_EMAIL, {
                                    ...emailData, 
                                    to: email ,
                                    campaignId,
                                    emailCampaignId,
                                    finalEmail: clientEmails.size === count
