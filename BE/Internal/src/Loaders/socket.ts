...
class SocketConnection {
    public async init(io: any) {
        ...
        io.on("connection", (socket: any) => {
            socket.on("client:message:send", (channelId: string, message: string) => {
                socket.to('Channel_' + channelId).emit('message:send:fromClient',channelId, message)
            });
            socket.on("staff:message:send", (channelId: string, message: string) => {
                socket.to('Channel_' + channelId).emit('message:send:fromStaff',channelId, message)
            });
            ...
            socket.on("staff:message:read", (channelId: string) => {
                ...
                socket.to('Channel_' + channelId).emit('message:read:fromStaff',channelId)
            });
        });
    }
}
