...
import type { Key, SortOrder } from "antd/es/table/interface";
import {
    SearchOutlined,
...
import { CreateModal } from "../TableComponents/CreateModal";
...
const TableOrderRender = <T extends AnyObject>({
    columns,
    url,
    onSelected,
    ...props
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const respone = await fetchClient({
                url: `${url}?page=${tableParams.pagination?.current}
				&pageSize=${tableParams.pagination?.pageSize}${sortQueries}${filterQueriesStr}${
                    props.queryStr ? `&${props.queryStr}` : ""
                }`,
            });
            ...
            if (props.excludeDataHasIds) {
                data = data.filter(
                    (item: any) => !props.excludeDataHasIds?.includes(item.id)
                );
            }
    ...
    const handleApplyFilter = (values: any) => {
        ...
        for (let key of Object.keys(values)) {
            if (
                ["startsWith", "endsWith", "like", "contains"].includes(
                    values[key].expr
                )
            ) {
                queryStr = values[key].value
                    ? `${queryStr}&${key}_${values[key].expr}=${values[key].value}`
                    : queryStr;
            } else if (
                ["today", "last7day", "currentMonth"].includes(values[key].expr)
            ) {
                const timeQuery = timeConvert(values[key].expr);
                queryStr = `${queryStr}&${key}_gt=${timeQuery}`;
            } else if (["lastXDays"].includes(values[key].expr)) {
                const timeQuery = timeConvert(
                    values[key].expr,
                    Number(values[key].value)
                );
                queryStr = values[key].value
                    ? `${queryStr}&${key}_gt=${timeQuery}`
                    : queryStr;
            } else if (["before", "after"].includes(values[key].expr)) {
                queryStr = values[key].value
                    ? `${queryStr}&${key}_gt=${values[key].value.toString()}`
                    : queryStr;
    ...
    const remainingFilterItems = props.filterItems
        ...
        >
            <div className='w-full h-auto flex-col gap-3'>
                ...
                <div className='border bg-white shadow p-3 my-2 rounded'>
                    ...
                    {props.filterItems && (
                        <Form onFinish={handleApplyFilter}>
                            <div className='flex justify-start gap-2 flex-wrap'>
                                {filterItems.map((item) => (
                                    <FilterItem
                                        ...
                                        onDelete={() =>
                                            handleDeleteFilteritem(item.key)
                                        }
                                    />
                                ))}
                            </div>
...
export default TableOrderRender;
