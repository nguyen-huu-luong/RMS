...
import { JobType, QUEUE_NAMES, QueueService } from "../Services/Queue.service";
...
class MarketingController {
	...
	public async sendEmail(req: Request, res: Response, next: NextFunction) {
		...
		const queue = new QueueService().getQueue(QUEUE_NAMES.DEFAULT);
		for (let email of receivers) {
			...
			queue.add(JobType.SEND_EMAIL, emailData, { attempts: 3 });
		}
