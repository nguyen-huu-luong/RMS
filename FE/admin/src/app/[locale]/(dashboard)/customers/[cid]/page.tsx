...
const CustomerProfile = () => {
    ...
    const historyTitle: any = {
        add_to_cart: {
            title: "Add To Cart",
            color: "geekblue",
            link: "../bussiness/products/dishes",
    ...
    const fetchData = async () => {
        ...
        const data = await fetchClient({
            url: `/customers/${params.cid}`,
            data_return: true,
        });
        const histories = await fetchClient({
            url: `/clienthistories/${params.cid}`,
            data_return: true,
        });
    ...
    if (isLoading) {
        ...
        if (customerInfo) {
            userInfo = {
                ...
                gender: String(customerInfo.gender),
                phone: customerInfo.phone,
                birthday: customerInfo.birthday
                    ? customerInfo.birthday.split("T")[0]
    ...
    const users_activity: TableProps<CustomerHistoryDataType>["columns"] = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <a
                    style={{ color: "#4A58EC" }}
                    href={historyTitle[record.action].link}
                >
                    {historyTitle[record.action].reference}{" "}
                    {record.orderId ? record.orderId : record.productId}
                </a>
            ),
        },
        {
    ...
    const columns_order: TableProps<OrderDataType>["columns"] = [
        {
            ...
            key: "id",
            render: (text) => (
                <a style={{ color: "#4A58EC" }} href='../sale/orders'>
                    {text}
                </a>
            ),
        },
            ...
            render: (_, { status }) => (
                <>
                    {status.map((status_item) => {
                        let color =
                            status_item.length > 5 ? "geekblue" : "green";
    ...
    const orderData: OrderDataType[] = customerInfo?.orderInfo.map(
        (order: any) => {
            return {
                key: order.id,
                id: order.id,
    ...
    const totalCost = costs?.reduce(
        (cost: any, initial: any) => cost + initial,
        0
    );
                    ...
                    {editFlag && (
                        <div className='inline-block'>
                            ...
                            <button
                                ...
                                onClick={() =>
                                    router.push(`/chat?id=${customerInfo.id}`)
                                }
                            >
                                <MessageOutlined />
                            </button>
                                        ...
                                        <div>
                                            {customerInfo.group.length > 0 ? (
                                                <Tooltip
                                                    placement='top'
                                                    title={
                                                        customerInfo.group[0]
                                                            .description
                                                    }
                                                >
                                                    <button
                                                        className='inline-flex items-center rounded-md px-2 py-1 text-xs font-medium text-gray-600 '
                                                        style={{
                                                            backgroundColor:
                                                                "#E7E9FD",
                                                            color: "#4A58EC",
