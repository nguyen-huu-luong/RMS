...
export const LeadActionBar: React.FC<LeadActionBarProps> = (props) => {
    ...
    return <main className="bg-white w-full py-2 px-3 rounded-md border">
        <Flex>
            {
                (props.dataSelected && props.dataSelected?.length > 0) ? 
                    <Space>
                        <p>Selected {props.dataSelected?.length} lead</p>
        ...
        <Modal
            ...
            footer={null}
        >
        <AddLeadForm  afterSubmit={handleOk} afterCancel={handleCancel}/>
    </Modal>
